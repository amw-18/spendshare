...
class User(SQLModel, table=True):
    ...
    expenses_participated_in: List["Expense"] = Relationship(
        back_populates="participants", link_model=ExpenseParticipant
    )
...
class Group(SQLModel, table=True):
    ...
    created_by: User = (
        Relationship()
    )  # Will be User, but need to define User first or use string
...
class Expense(SQLModel, table=True):
    ...
    participants: List[User] = Relationship(
        back_populates="expenses_participated_in", link_model=ExpenseParticipant
    )
