...
async def test_create_simple_expense_success(client: AsyncClient):
    ...
    assert login_res.status_code == status.HTTP_200_OK, (
        f"Failed to log in payer: {login_res.text}"
    )
    ...
    response = await client.post(
        "/api/v1/expenses/", json=expense_data, headers=payer_headers
    )  # Added headers
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
    ...
    )  # Check for participant_details as it's in ExpenseRead
...
@pytest.mark.asyncio
async def test_create_expense_normal_user(
...
async def test_read_expense_authorization(
    ...
    assert (
        response_payer_view.json()["description"]
        == expense_payload["expense_in"]["description"]
    )
