...
from app.tests.conftest import test_engine # Import the test_engine from conftest

@pytest.mark.asyncio
async def test_read_root(client: AsyncClient):
    response = await client.get("/api/v1/") # Assuming /api/v1 is the prefix for the root in main
...
async def test_db_tables_created_on_startup(client: AsyncClient):
    ...
    assert await table_exists("usergrouplink", test_engine) # Check for 'usergrouplink' table
    assert await table_exists("expenseparticipant", test_engine) # Check for 'expenseparticipant' table
