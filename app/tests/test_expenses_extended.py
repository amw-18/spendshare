...
@pytest.mark.asyncio
async def test_create_expense_negative_amount(client: AsyncClient, normal_user_token_headers: dict[str, str]):
    ...
    assert "input should be greater than 0" in response.json()["detail"][0]["msg"].lower()

@pytest.mark.asyncio
async def test_create_expense_zero_amount(client: AsyncClient, normal_user_token_headers: dict[str, str]):
...
@pytest.mark.asyncio
async def test_create_expense_empty_description(client: AsyncClient, normal_user_token_headers: dict[str, str]):
...
@pytest.mark.asyncio
async def test_expense_filter_by_user(client: AsyncClient, normal_user_token_headers: dict[str, str], normal_user: Any):
    ...
    await client.post("/api/v1/expenses/", json=expense_data1, headers=normal_user_token_headers)
    await client.post("/api/v1/expenses/", json=expense_data2, headers=normal_user_token_headers)
    
    # Filter by user_id
    response = await client.get(f"/api/v1/expenses/?user_id={normal_user.id}", headers=normal_user_token_headers)
    assert response.status_code == status.HTTP_200_OK
...
@pytest.mark.asyncio
async def test_expense_filter_by_group(client: AsyncClient, normal_user_token_headers: dict[str, str], normal_user: Any):
    ...
    # Filter by group_id
    response = await client.get(f"/api/v1/expenses/?group_id={group_id}", headers=normal_user_token_headers)
    assert response.status_code == status.HTTP_200_OK
    expenses = response.json()
...
@pytest.mark.asyncio
async def test_expense_pagination(client: AsyncClient, normal_user_token_headers: dict[str, str]):
    ...
    # Test with limit
    response = await client.get("/api/v1/expenses/?limit=2", headers=normal_user_token_headers)
    assert response.status_code == status.HTTP_200_OK
    expenses = response.json()
    assert len(expenses) == 2
    
    # Test with skip
    response = await client.get("/api/v1/expenses/?skip=2&limit=2", headers=normal_user_token_headers)
    assert response.status_code == status.HTTP_200_OK
    expenses = response.json()
    assert len(expenses) == 2

@pytest.mark.asyncio
async def test_expense_share_calculation(client: AsyncClient, normal_user_token_headers: dict[str, str], normal_user: Any):
    """Test expense share calculation for multiple participants"""
    # Create another user as participant
    participant = await create_test_user(client, "share_participant", "share_part@example.com")
...
@pytest.mark.asyncio
async def test_expense_update_validation(client: AsyncClient, normal_user_token_headers: dict[str, str], normal_user: Any):
    ...
    create_response = await client.post("/api/v1/expenses/", json=expense_data, headers=normal_user_token_headers)
    ...
    response_neg_amount = await client.put(f"/api/v1/expenses/{expense_id}", json=update_data_neg_amount, headers=normal_user_token_headers)
    ...
    assert any(err["type"] == "greater_than" and err["loc"] == ["body", "amount"] for err in response_neg_amount.json()["detail"])
    ...
    response_empty_desc = await client.put(f"/api/v1/expenses/{expense_id}", json=update_data_empty_desc, headers=normal_user_token_headers)
    ...
    assert any(err["type"] == "string_too_short" and err["loc"] == ["body", "description"] for err in response_empty_desc.json()["detail"])
