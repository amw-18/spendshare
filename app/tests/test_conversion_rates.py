...
from sqlalchemy.ext.asyncio import AsyncSession # Though session might be handled by fixtures/helpers
from fastapi import status
from typing import List, Optional

# Models and Schemas
from src.models.models import User, Currency, ConversionRate # Corrected path
from src.models.schemas import ConversionRateRead, CurrencyRead # Corrected path

# Assuming conftest.py provides these fixtures:
# client: AsyncClient
# db_setup_session: AsyncSession (for setup/teardown, not direct use in helpers)
# normal_user: User
# admin_user: User
# normal_user_token_headers: dict
# admin_user_token_headers: dict
# TestingSessionLocal: for creating sessions in helpers if needed

from tests.conftest import TestingSessionLocal # Corrected path: To create sessions in helpers
...
async def test_create_conversion_rate_as_admin( # Renaming to test_create_conversion_rate
    ...
    eur = await create_test_currency_for_rates(code="EUR", name="Euro", symbol="€")
    ...
    assert data["from_currency"]["code"] == "USD"
    assert data["to_currency"]["id"] == eur.id
    assert data["to_currency"]["code"] == "EUR"
    assert "id" in data
...
async def test_create_conversion_rate_as_normal_user(
    ...
    usd = await create_test_currency_for_rates(code="USX", name="US Dollar X") # Use different code to avoid conflicts if tests run in parallel or DB is not perfectly clean
...
async def test_read_conversion_rates_with_data( # Setup can be done by normal user
    ...
    codes_in_response = [ (item['from_currency']['code'], item['to_currency']['code']) for item in data ]
    ...
    if len(data) >= 2: # Check order if possible
        ...
        idx_cr2_cr3 = next((i for i, item in enumerate(data) if item['from_currency']['code'] == curr2.code and item['to_currency']['code'] == curr3.code), -1)
        idx_cr1_cr2 = next((i for i, item in enumerate(data) if item['from_currency']['code'] == curr1.code and item['to_currency']['code'] == curr2.code), -1)
...
async def test_read_conversion_rates_pagination( # Setup can be done by normal user
    ...
    if len(all_rates) > 1:
        ...
        assert data_skip_1[0]['id'] != all_rates[0]['id'] # Assuming default order by timestamp desc
...
async def test_read_latest_conversion_rate_success( # Setup can be done by normal user
    ...
    response = await client.get(f"/api/v1/conversion-rates/latest?from_code={curr_L1.code}&to_code={curr_L2.code}")
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
async def test_read_latest_conversion_rate_not_found(client: AsyncClient):
    response = await client.get("/api/v1/conversion-rates/latest?from_code=NONEXIST1&to_code=NONEXIST2")
    assert response.status_code == status.HTTP_404_NOT_FOUND # Should be 404 if codes don't exist
...
async def test_read_latest_conversion_rate_pair_not_found(client: AsyncClient):
    ...
    response = await client.get(f"/api/v1/conversion-rates/latest?from_code={curr_NE1.code}&to_code={curr_NE2.code}")
    assert response.status_code == status.HTTP_404_NOT_FOUND # Should be 404 if pair has no rates

@pytest.mark.asyncio
async def test_read_latest_conversion_rate_same_currency(client: AsyncClient):
    curr_SC = await create_test_currency_for_rates(code="CSAM", name="Same Currency Test")
    response = await client.get(f"/api/v1/conversion-rates/latest?from_code={curr_SC.code}&to_code={curr_SC.code}")
    assert response.status_code == status.HTTP_400_BAD_REQUEST
