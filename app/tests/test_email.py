...
@patch("app.src.core.email.requests.post")
def test_send_email_mailgun_success(mock_post, mock_settings_mailgun_configured):
...
@patch("app.src.core.email.requests.post")
def test_send_email_mailgun_api_failure(mock_post, mock_settings_mailgun_configured):
...
@patch("app.src.core.email.requests.post")
def test_send_email_mailgun_request_exception(mock_post, mock_settings_mailgun_configured):
    """Test scenario where requests.post raises an exception (e.g., network error)."""
    mock_post.side_effect = requests.exceptions.RequestException("Simulated connection error")
...
def test_send_email_mailgun_not_configured_apikey_domain(mock_settings_mailgun_not_configured):
    ...
    with patch("app.src.core.email.requests.post") as mock_post: # Ensure no call is made
        result = send_email_mailgun(
            email_to=TEST_EMAIL_TO,
            subject=TEST_SUBJECT,
            html_content=TEST_HTML_CONTENT
...
def test_send_email_mailgun_not_configured_from_email(mock_settings_mailgun_no_from_email):
...
@pytest.mark.asyncio
@patch("app.src.core.email.send_email_mailgun") # Mock the actual call to Mailgun
