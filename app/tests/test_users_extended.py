...
async def test_password_validation(client: AsyncClient):
    ...
    assert "string should have at least 8 characters" in response.json()["detail"][0]["msg"].lower()
    
    # Test password without numbers
    user_data["password"] = "onlyletters"
    response = await client.post("/api/v1/users/", json=user_data)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
    assert "password must contain at least one number" in response.json()["detail"][0]["msg"].lower()
    
    # Test password without letters
    user_data["password"] = "12345678"
    response = await client.post("/api/v1/users/", json=user_data)
    assert response.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY
    assert "password must contain at least one letter" in response.json()["detail"][0]["msg"].lower()
...
@pytest.mark.asyncio
async def test_user_listing_pagination(client: AsyncClient, admin_user_token_headers: dict[str, str]):
    ...
    # Test with limit
    response = await client.get("/api/v1/users/?limit=2", headers=admin_user_token_headers)
    assert response.status_code == status.HTTP_200_OK
    users = response.json()
    assert len(users) == 2
    
    # Test with skip
    response = await client.get("/api/v1/users/?skip=2&limit=2", headers=admin_user_token_headers)
    assert response.status_code == status.HTTP_200_OK
    users = response.json()
    assert len(users) == 2

@pytest.mark.asyncio
async def test_password_update_validation(client: AsyncClient, normal_user_token_headers: dict[str, str], normal_user: Any):
...
@pytest.mark.asyncio
async def test_concurrent_user_updates(client: AsyncClient, normal_user_token_headers: dict[str, str], admin_user_token_headers: dict[str, str], normal_user: User):
    ...
    response = await client.get(f"/api/v1/users/{normal_user.id}", headers=admin_user_token_headers)
    assert response.status_code == status.HTTP_200_OK
