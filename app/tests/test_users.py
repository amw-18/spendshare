...
# from app.models import schemas # For response model validation if desired, e.g. schemas.UserRead

# Keep or modify the initial check
@pytest.mark.asyncio
async def test_read_users_empty_db(client: AsyncClient):
...
async def test_create_user_success(client: AsyncClient):
    ...
    assert response.status_code == status.HTTP_200_OK # FastAPI default is 200 for POST if not specified
    data = response.json()
...
async def test_create_user_duplicate_email(client: AsyncClient):
    ...
    response2 = await client.post("/users/", json=user_data_duplicate_email)
    assert response2.status_code == status.HTTP_400_BAD_REQUEST
    assert "user with this email already exists" in response2.json()["detail"].lower() # Adjusted to match router message
...
async def test_create_user_duplicate_username(client: AsyncClient):
    ...
    response2 = await client.post("/users/", json=user_data_duplicate_username)
    assert response2.status_code == status.HTTP_400_BAD_REQUEST
    assert "user with this username already exists" in response2.json()["detail"].lower() # Adjusted

@pytest.mark.asyncio
async def test_read_user_not_found(client: AsyncClient):
    response = await client.get("/users/99999") # Non-existent user ID
...
async def test_read_users_with_data(client: AsyncClient):
    ...
    resp1 = await client.post("/users/", json=user1_data)
    assert resp1.status_code == status.HTTP_200_OK
    resp2 = await client.post("/users/", json=user2_data)
...
async def test_update_user_not_found(client: AsyncClient):
    ...
    response = await client.put("/users/99999", json=update_data) # Non-existent user ID
    assert response.status_code == status.HTTP_404_NOT_FOUND

@pytest.mark.asyncio
async def test_update_user_email_conflict(client: AsyncClient):
    user1 = {"username": "user1_conflict", "email": "user1_conflict@example.com", "password": "pw"}
    user2 = {"username": "user2_conflict", "email": "user2_conflict@example.com", "password": "pw"}
    resp1 = await client.post("/users/", json=user1)
    assert resp1.status_code == status.HTTP_200_OK
    resp2 = await client.post("/users/", json=user2)
    assert resp2.status_code == status.HTTP_200_OK
    user2_id = resp2.json()["id"]

    update_data = {"email": user1["email"]} # Try to update user2's email to user1's email
    response = await client.put(f"/users/{user2_id}", json=update_data)
...
async def test_delete_user_not_found(client: AsyncClient):
    response = await client.delete("/users/99999") # Non-existent user ID
    assert response.status_code == status.HTTP_404_NOT_FOUND
