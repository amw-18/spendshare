...
user_id_cookie = "session_user_id"
...
async def get_home_page(request: Request):
    ...
    if user_id:  # If "logged in", go to dashboard
        return RedirectResponse(
            url=request.url_for("get_dashboard_page"), status_code=status.HTTP_302_FOUND
        )
    return RedirectResponse(
        url=request.url_for("get_login_page"), status_code=status.HTTP_302_FOUND
    )  # Else, go to login
...
async def handle_signup(
    ...
    if db_user_by_email:
        return RedirectResponse(
            url=request.url_for("get_signup_page").include_query_params(
    ...
    return RedirectResponse(
        url=request.url_for("get_login_page").include_query_params(
...
async def get_login_page(
    ...
    return request.app.state.templates.TemplateResponse(
        "login.html", {"request": request, "message": message, "error": error}
    )
...
async def handle_login(
    ...
    response = RedirectResponse(
        url=request.url_for("get_dashboard_page"), status_code=status.HTTP_303_SEE_OTHER
    )
    response.set_cookie(
        key=user_id_cookie,
        value=str(user.id),
...
async def handle_logout(request: Request):
    ...
    response.delete_cookie(key=user_id_cookie, httponly=True, samesite="lax")
    return response
...
async def get_dashboard_page(
    ...
    user_id_str = request.cookies.get(user_id_cookie)
    ...
    if not current_user:
        return RedirectResponse(
            url=request.url_for("get_login_page").include_query_params(
                error="Please login to view the dashboard."
            ),
            status_code=status.HTTP_302_FOUND,
        )

    all_other_users = await crud_user.get_users(
        session=session, skip=0, limit=1000
    )  # Fetch users for "add friends" simulation
    ...
    created_groups = await crud_group.get_groups_created_by_user(
        session=session, user_id=current_user.id
    )
    member_of_groups = await crud_group.get_groups_for_member(
        session=session, user_id=current_user.id
    )
...
async def handle_create_group(
    ...
    if not user_id_str:
        return RedirectResponse(
            url=request.url_for("get_login_page").include_query_params(
                error="Please login to create groups."
            ),
            status_code=status.HTTP_302_FOUND,
        )
    ...
    await crud_group.create_group(session=session, group_in=group_in)
    return RedirectResponse(
        url=request.url_for("get_dashboard_page").include_query_params(
...
@router.post(
    ...
    name="handle_add_member_to_group",
...
async def handle_add_member_to_group(
    ...
    requesting_user_id_str = request.cookies.get(user_id_cookie)
    ...
    try:
        updated_group = await crud_group.add_member_to_group(
            session=session, group_id=group_id, user_id=user_id_to_add
        )
        if (
            updated_group is None
        ):  # Should not happen if crud raises exceptions for not found
            return RedirectResponse(
                url=request.url_for(
                    "get_group_page", group_id=group_id
                ).include_query_params(
                    error="Failed to add member. Group or user not found."
                ),
                status_code=status.HTTP_303_SEE_OTHER,
            )
        return RedirectResponse(
            url=request.url_for(
                "get_group_page", group_id=group_id
            ).include_query_params(message="Member added successfully."),
            status_code=status.HTTP_303_SEE_OTHER,
        )
        ...
        return RedirectResponse(
            url=request.url_for(
                "get_group_page", group_id=group_id
            ).include_query_params(error=f"{e.detail.replace(' ', '+')}"),
            status_code=status.HTTP_303_SEE_OTHER,
        )
        ...
        return RedirectResponse(
            url=request.url_for(
                "get_group_page", group_id=group_id
            ).include_query_params(error="An unexpected error occurred."),
            status_code=status.HTTP_303_SEE_OTHER,
        )
